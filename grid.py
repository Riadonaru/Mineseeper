...
class Grid():
    ...
    def __init__(self) -> None:
        ...
        self.temp = np.reshape(
            self.temp, (SETTINGS["height"], SETTINGS["width"]), 'F')

        # Populating the array with mines
        self.contents = np.ndarray(
            (SETTINGS["height"], SETTINGS["width"])).astype(Cell)
    ...
    def easy_start(self, clicked_x: int, clicked_y: int):
        if not self.contents[clicked_y][clicked_x].flagged:
            maxy, maxx = self.contents.shape
            for dy in range(-1, 2):
                for dx in range(-1, 2):
                    if 0 <= clicked_y + dy < maxy and 0 <= clicked_x + dx < maxx and self.contents[clicked_y + dy][clicked_x + dx].value == MINE:
                        x_new, y_new = self.find_clear_spot()
                        if x_new == -1 and y_new == -1:
                            self.troll_mode = True
                            self.troll()
                            return
                        self.contents[y_new][x_new].value = MINE
            ...
            for dy in range(-1, 2):
                for dx in range(-1, 2):
                    if 0 <= x_index + dx < SETTINGS["width"] and 0 <= y_index + dy < SETTINGS["height"]:
                        adj_cell: Cell = self.contents[y_index +
                                                       dy][x_index + dx]
